...
public class GraphComponent extends JComponent {
	...
	/**
	 * An inner class that represent a location of a node. Fields cX and cY are the center of the
	 * node, and fields tX and tY are the location where the label of the node is drawn. This is
	 * calculated for each node in the method {@code calculateNodeCoordinates()}
	 *
	 */
	private class Point {
		int cX;
	...
	/**
	 * width and height of the window when it was last resized. When change we recalculate the
	 * location of nodes to scale the graph, etc.
	 */
	...
	private void drawMap(Graphics g) {
		...
		for (Edge e : _graph.getEdges()) {
			...
			for (Dot d : e.getDots()) {
				...
				drawCircleOnALine(g, p1.cX, p1.cY, p2.cX, p2.cY, e.getLength(), d.getLocation(),
						diam, dotColor, d.getId());
			}
		}
	}

	/**
	 * put the objects in a circle, for each one store the center coordinate and a coordinate for a
	 * corresponding text.
	 */
	...
	/**
	 * Draws a circle on the line from (x1,y1) to (x2,y2). Assuming the (virtual) length of the line
	 * is virtualLength, the circles is drawn at location virtualLocation (0..virtualLength). The
	 * diameter is 'diam'
	 */
	...
	/**
	 * Draws a line from (x1,y1) to (x2,y2) with an arrow of width d and height h. The color of the
	 * line is lineColor and that of the arrow is arrowColor.
	 */

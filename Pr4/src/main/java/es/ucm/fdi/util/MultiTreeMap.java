 ...
 *
 * Values for the same key will be returned and traversed in order of insertion; that is, newer
 * values with the same key will be stored after any other values with the same key.
 */
public class MultiTreeMap<K, V> extends TreeMap<K, ArrayList<V>> {
	...
	/**
	 * Removes the first occurrence of a value from the list found at a given key. Efficiency is
	 * O(size-of-that-list)
	 * 
	 * @param key
	 *            to look into
	...
	/**
	 * Iterates through all internal values (not the arraylists themselves), first by key order, and
	 * within each bucket, by insertion order.
	 */

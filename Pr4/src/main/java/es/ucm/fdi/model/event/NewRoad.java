...
public class NewRoad extends Event {
	...
	protected void checkParameters() throws SimulationException {
		...
		if (length <= 0) {
			throw new SimulationException("Error en la ejecucion del evento de creacion de la carretera " + id + " la longitud ha de ser estrictamente positiva.");
		}
	}
	...
	public class Builder implements Event.Builder{
		...
		public Event parse(IniSection sec) throws IOException {
			...
			if(sec.getValue("src") == null || sec.getValue("dest") == null) throw new IOException("La carretera " + sec.getValue("id") + " esta mal definida en el archivo de lectura.");
			else return new NewRoad(parseInt(sec, "time", 0),isValidId(sec.getValue("id")), sec.getValue("src"), sec.getValue("dest"), parseInt(sec, "max_speed", 0), parseInt(sec, "length", 0));
		}
	}

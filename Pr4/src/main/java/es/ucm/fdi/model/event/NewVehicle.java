...
public class NewVehicle extends Event {
	...
	protected void checkParameters() {
		if (maxSpeed <= 0) {
			throw new SimulationException("Error en la ejecucion del evento creador del vehiculo "
					+ id + " la velocidad maxima ha de ser estrictamente positiva.");
		}

		if (itinerary.size() < 2) {
			throw new SimulationException("Error en la ejecucion del evento " + id
	...
	public class Builder implements Event.Builder {
		...
		public Event parse(IniSection sec) {
			if (!sec.getTag().equals("new_vehicle"))
				return null;
			return new NewVehicle(parseInt(sec, "time", 0), isValidId(sec.getValue("id")),

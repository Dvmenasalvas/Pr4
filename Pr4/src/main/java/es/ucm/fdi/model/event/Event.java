...
/**
 * Un evento es un nuevo suceso que le llega al simulador en el turno time un evento puede ser o
 * bien un nuevo SimObject o bien un que un vehículo se ha averiado, Event es una clase abstracta
 * que implementa los métodos comunes que las clases hijas necsitan para leer en formato INI los
 * parámetros de cada evento concreto, tales como leer enteros, leer id, etc Además en cada clase
 * que herede de Event debe haber implementado un método execute que añada al RoadMap el nuevo
 * objeto y un método checkParameters que se asegure de que los parámetros necesarios para
 * determinar el evento estén completos y sean coherentes con las reglas del programa
 */
